...
;#include "m80_cp437.s"
;#include "m80_mode9.s"
;#include "m80_terminal.s"
#include "m80_terminal_uze.s"

; Cursor struct field offsets
#define CUR_X			0
#define CUR_Y			1
#define CUR_BLINKRATE	2
#define CUR_CURRDELAY	3
#define CUR_STATE		4
#define CUR_BAKTILE		5
#define CUR_BAKADDR		6
#define CUR_ACTIVE		8
...
;
	...
	lds		ZL,		m80_cursor + 0
	lds		ZH,     m80_cursor + 1
	ldd		r24,	Z+CUR_ACTIVE
	cpi 	ZH,0
	brne .+2
	clr 	r24
	cpi 	r24,0
	breq	cur_off	;branch if cursor not active or pointer not set

	ldi 	r22,	0x80
	ldd		r23,	Z+CUR_STATE
	ldd		r24,	Z+CUR_CURRDELAY
	ldd		r25,	Z+CUR_BLINKRATE
	dec		r24		;decrementer delay counter
	brpl .+2
	mov 	r24,r25	;reload delay if counter <0
	brpl .+2
	eor		r23,r22	;flip display state if counter <0
	std		Z+CUR_CURRDELAY, r24
	std		Z+CUR_STATE, r23

	;compute adress of tile under cursor
	...
	brlo .+2
	subi	r24,	VRAM_TILES_V	;wrap
	ldi		r25,	VRAM_TILES_H
	mul		r24,	r25
	ldi		YL,		lo8(vram)
	ldi		YH,		hi8(vram)
	add		YL,		r0
	...
	ld		r24,	Y	;load tile under cursor
	std		Z+CUR_BAKTILE,r24
	std		Z+CUR_BAKADDR,YL
	std		Z+CUR_BAKADDR+1,YH

	cpi 	r23,	0	;cursor visible?
	breq .+2
	eor		r24,	r22	;flip msbit if cursor visible
	st		Y,		r24	;store back to vram
	rjmp cur_end
cur_off:
	WAIT  r18, 57
cur_end:
...
scl_0e:
	...
	lds		ZL,		m80_cursor + 0
	lds		ZH,     m80_cursor + 1
	ldd		r24,	Z+CUR_BAKTILE
	ldd		YL,		Z+CUR_BAKADDR+0
	ldd		YH,		Z+CUR_BAKADDR+1
	st		Y,		r24
