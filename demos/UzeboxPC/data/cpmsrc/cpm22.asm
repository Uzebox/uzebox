...
BOOTMSG  DB   0CH,"Uzebox 64K CP/M Version 2.2\r\nCopyright (c) 1979 Digital Research, Inc.\r\n\r\n",0
...
;77 tracks, 26 sectors per track, 128 byte per sector, skew 6

dpblk:	;disk parameter block, common to all disks
	spt: dw	26	;Number of 128-bytes sectors per track
	bsh: db	3	;Block shift/size. 3 => 1k, 4 => 2k, 5 => 4k....
	blm: db	7	;Block mask. 7 => 1k, 0Fh => 2k, 1Fh => 4k...
	exm: db	0	;Extent mask, see later
	dsm: dw	250	;(no. of blocks on the disc)-1
	drm: dw	63	;(no. of directory entries)-1
	al0: db	192	;Directory allocation bitmap, first byte
	al1: db	0	;Directory allocation bitmap, second byte
	cks: dw	16	;Checksum vector size, 0 for a fixed disc. No. directory entries/4, rounded up.
	off: dw	2	;Offset, number of reserved tracks

;	scratch ram area for bdos use
dirbf:	ds	128	 	;scratch directory area
all00:	ds	31	 	;allocation vector for drive 0
chk00:	ds	16		;check vector for drive 0
all01:	ds	31	 	;allocation vector for drive 1
chk01:	ds	16		;check vector for drive 1
all02:	ds	31	 	;allocation vector for drive 2
chk02:	ds	16		;check vector for drive 2
...
;
boot1:
	lxi		d,BOOTMSG   ;display sign-on message
btmsgloop:
	ldax d
	inx d
	out	01H			;send to console
	cpi 0			;end of line?
	jnz btmsgloop
...
;
wboot1:
	;TODO: relod the CCP and BDOS from disk...
...
gocpm:
	...
	jmp	CBASE		;transfer to CCP for further processing
...
seldsk1:
	...
	sta	DISKNO
	out 0AH
	cpi	0		;must be 0, only one drive supported
	rnz
...
read1:
	mvi a,0
	out 0DH
	in  0EH			;error condition of last fdc operation, 0=sucess, 1=fail
	ret
write1:
	nop
