...
//10,0,7,OP_GOTO, 50, 0, EOL, //0 to 500
10,0,12,OP_LD_BYTE, 1, OP_LD_BYTE, 0, OP_LD_BYTE, 2, OP_LOOP_FOR, VAR_A, EOL,
30,0,9,OP_PRINT_LSTR,' ','Y','o',EOSNL,EOL,
40,0,6,OP_LOOP_NEXT, VAR_A, EOL,
50,0,12,OP_PRINT_LSTR,' ','D','o','n','e','!',EOSNL,EOL,
0,0
};
...
static u16 spi_prog_pos,spi_current_line_pos,spi_current_line_no, current_line_len;
static u8*  spi_line_buf_pos;
...
u16 execute(bool initialize){
	...
	do{
		...
		SpiRamSeqReadStart(0,spi_prog_pos);
		spi_current_line_pos=spi_prog_pos;
		SpiRamSeqReadInto(spi_line_buf,3); //read line number (word) and line lenght (byte) in ram buffer
		current_line_no=(u16)spi_line_buf[0];
		current_line_len=(u8)spi_line_buf[2];
		SpiRamSeqReadInto(&spi_line_buf[3],current_line_len-3+2); //read remaining of line in ram buffer + two more byte for the next line number
		spi_line_buf_pos=&spi_line_buf[3]; ////skip line number and line lenght
		spi_prog_pos+=current_line_len;
		SpiRamSeqReadEnd();
		//<--
		...
		while(1){
			...
			switch(opcode){
				...
				case OP_LD_WORD:
					...
					//txtpos+=2;
					//-->
					u16_var=(*((int16_t*)spi_line_buf_pos));
					spi_line_buf_pos+=2;
				...
				case OP_LD_DWORD:
					...
					//txtpos+=4;
					//-->
					type_converter.i=(*((int32_t*)spi_line_buf_pos));
					spi_line_buf_pos+=4;
				...
				case OP_LD_VAR:
					//c1=*txtpos++;
					//-->
					c1=*spi_line_buf_pos++;	//Get variable number
				...
				case OP_ASSIGN:
					//Get variable number to assign to
					//c1=*txtpos++;
					//-->
					c1=*spi_line_buf_pos++;
				...
				case OP_LOOP_FOR:
					...
					//-->
					c1=*spi_line_buf_pos++; //get variable index (0-25)
					...
					//-->
					sf->spi_prog_pos= spi_prog_pos;
					sf->spi_current_line_pos=spi_current_line_pos;
				...
				case OP_IF:
					...
					if(!v1){
						...
						//-->>
						spi_line_buf_pos=&spi_line_buf[current_line_len-1];
			...
			//-->
			if(*spi_line_buf_pos == EOL || jump || stop)break;
	...
	//-->
	}while(*((u16*)spi_line_buf_pos)); //did we reached the end of the program (current line no != 0?)
