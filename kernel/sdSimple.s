...
; void mmcDirectReadSimple(uint8_t *dest, uint16_t count);
...
; Inputs
;     r24    - SD Card Command  (could be pre ORd to save a word)
...
sd_card_init_cmd0_loop:
	...
	rjmp    sd_card_clock_and_release_with_error       ; so we give up and return an error (0x01)
    ; phantom ret
...
sd_card_init_cmd1_done:
    ...
    rjmp    sd_card_deassert_cs             ; Deassert CS and return 0x00 "success" (already in r24 from above)
    ; phantom RET
