...
void LoadRootDirectory(unsigned char *buffer){
	...
	//read boot record
	long bootRecordSector= ((MBR*)buffer)->partition1.startSector;
	mmc_readsector(bootRecordSector, buffer);

	int reservedSectors=((BootRecord*)buffer)->reservedSectors;
	int sectorsPerFat=((BootRecord*)buffer)->sectorsPerFat;
...
unsigned char LoadFiles(unsigned char *buffer,File *files){
	//find files in the sector
	unsigned char i,j,k,pos,fileCount=0,c;
	DirectoryTableEntry *dirEntry;

	for(i=0;i<16;i++){
		dirEntry=&(((DirectoryTableEntry*)buffer)[i]);
