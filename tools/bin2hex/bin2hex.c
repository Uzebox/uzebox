 ...
 * Uzebox-specific changes made by Matt Pandina.
 *
...
char version[] = "xxd V1.10 27oct98 by Juergen Weigert, modified for Uzebox";
#ifdef WIN32
char osver[] = " (Win32)";
#else
# ifdef DJGPP
...
char *argv[];
{
  ...
  int cols = 0, nonzero = 0, autoskip = 0, hextype = HEX_CINCLUDE;
  int ebcdic = 0;
  ...
  if (hextype == HEX_CINCLUDE)
    {
      if (fp != stdin)
	{
	  fprintf(fpo, "const char %s", isdigit((int)argv[1][0]) ? "__" : "");
